...
class WCFM_Ajax {
	...
	public function __construct() {
		...
		// Add Series
    add_action('wp_ajax_wcfm_add_series', array( &$this, 'wcfm_add_series' ) );

		// Get Series
		add_action('wp_ajax_wcfm_get_series', array( &$this, 'wcfm_get_series' ) );

		// Get Genre Tags
		add_action('wp_ajax_wcfm_get_genre_tag', array( &$this, 'wcfm_get_genre_tag' ) );

		// Get Theme Tags
		add_action('wp_ajax_wcfm_get_theme_tag', array( &$this, 'wcfm_get_theme_tag' ) );
  ...
  function wcfm_add_series() {
	...
	function wcfm_get_series() {

		global $WCFM, $WCFMu, $_POST;
		$product_id = $_POST['id'];

		if(!empty($product_id)) {
			$author_id = get_post_field( 'post_author', (int)$product_id );
		} else {
			$author_id = apply_filters( 'wcfm_current_vendor_id', get_current_user_id() );
		}
		...
		$term_children = get_term_children((int)$parent_id, 'artists_cat');
		...
		if ( ! is_wp_error( $term_children ) ) {
			$response .= '{"categories": [';
				...
				foreach ( $term_children as $child ) {
					$term = get_term_by( 'id', $child, 'artists_cat' );
					if(!$is_first) $response .= ',';
