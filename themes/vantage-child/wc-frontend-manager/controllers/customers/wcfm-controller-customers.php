	...
	public function processing() {
		...
		if(!empty($wcfm_customers_array)) {
			foreach( $wcfm_customers_array as $wcfm_customers_single ) {
				...
				$avatar = get_avatar( $wcfm_customers_single->ID );
				if($avatar) {
					$wcfm_customers_json_arr[$index][] = '<div className="customer-avatar">' . $avatar . '</div>';
				}
				...
				if( apply_filters( 'wcfm_allow_view_customer_email', true ) ) {
					$wcfm_customers_json_arr[$index][] = '<span class="wcfm-customer-label">Email:</span>'.$wcfm_customers_single->user_email;
				} else {
					$wcfm_customers_json_arr[$index][] = '<span class="wcfm-customer-label">Email:</span>'.'&ndash;';
				}
				...
				if ( $value ) {
					$wcfm_customers_json_arr[$index][] = '<span class="wcfm-customer-label">Location: </span>'.$value;
				} else {
					$wcfm_customers_json_arr[$index][] = '<span class="wcfm-customer-label">Location: </span>'.'&ndash;';
				}

				// Orders
				$wcfm_customers_json_arr[$index][] = '<span class="wcfm-customer-label">Orders: </span>'.wc_get_customer_order_count( $wcfm_customers_single->ID );
				...
				// Money Spent
				$wcfm_customers_json_arr[$index][] = '<span class="wcfm-customer-label">Amount spent: </span>'.wc_price( wc_get_customer_total_spent( $wcfm_customers_single->ID ) );
				...
				if ( ! empty( $orders ) ) {
					...
					if( apply_filters( 'wcfm_is_allow_order_details', true ) && $WCFM->wcfm_vendor_support->wcfm_is_order_for_vendor( $order->get_id() ) ) {
						...
						$wcfm_customers_json_arr[$index][] = '<span class="wcfm-customer-label">Order No.: </span>'.'<span class="customer-orderno">' . _x( '#', 'hash before order number', 'woocommerce' ) . $order->get_order_number() . '</span><br />' . wc_format_datetime( $order->get_date_created() );
					}
				} else {
					$wcfm_customers_json_arr[$index][] = '<span class="wcfm-customer-label">Order No.: </span>'.'&ndash;';
				}

				// Additional Info
				$wcfm_customers_json_arr[$index][] = apply_filters( 'wcfm_customers_additonal_data', '&ndash;' );
