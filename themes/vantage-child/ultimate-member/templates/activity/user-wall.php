...
foreach ( $wallposts->posts as $post ) {
	...
	<div class="um-activity-widget" id="postid-<?php echo $post->ID; ?>">
		...
		<div class="um-activity-foot status" id="wallcomments-<?php echo $post->ID; ?>">
			<?php if ( is_user_logged_in() ) {
				$curr_page = add_query_arg( 'wall_post', $post->ID, UM()->permalinks()->get_current_url() );
				...
				<div class="um-activity-left um-activity-actions">
					...
					<?php } ?>
						<div class="um-activity-share">
							<?php if (function_exists('artmo_get_share_post_component')) { echo artmo_get_share_post_component($curr_page); } ?>
						</div>
				</div>
