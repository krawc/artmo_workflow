		...
		if(!empty($wcfm_memberships_array)) {
			...
			foreach($wcfm_memberships_array as $wcfm_memberships_single) {
				...
				if( $subscription_pay_mode == 'by_wc' ) {
					$pay_mode .= '<br /><a style="color: #6BBA70;" target="_blank" href="' . get_wcfm_edit_product_url( $subscription_product ) . '"><span class="fa fa-cube"></span>&nbsp;' . get_the_title( $subscription_product ) . '</a>';
				}
				...
				// Action
				$actions = '<a class="wcfm-action-icon" href="' . get_wcfm_memberships_manage_url($wcfm_memberships_single->ID) . '"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Edit', 'wc-frontend-manager' ) . '"></span></a>';
				if( $membership_users && is_array( $membership_users ) && ( count( $membership_users ) > 0 ) && apply_filters( 'wcfm_is_allow_block_membership_delete', true ) ) {
					$actions .= '<a class="wcfm_membership_restrict_delete wcfm-action-icon" href="#"><span class="fa fa-trash-o text_tip" data-tip="' . esc_attr__( 'Delete', 'wc-frontend-manager' ) . '"></span></a>';
				} else {
					$actions .= '<a class="wcfm_membership_delete wcfm-action-icon" href="#" data-membershipid="' . $wcfm_memberships_single->ID . '"><span class="fa fa-trash-o text_tip" data-tip="' . esc_attr__( 'Delete', 'wc-frontend-manager' ) . '"></span></a>';
				}
