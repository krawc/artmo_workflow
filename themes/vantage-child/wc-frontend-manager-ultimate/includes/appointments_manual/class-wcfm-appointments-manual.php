		...
		try {
			...
			if ( ! empty( $_POST['add_appointment'] ) ) {
				...
				$new_appointment_data = array(
					...
					'staff_ids'		=> isset( $appointment_data['_staff_ids'] ) ? $appointment_data['_staff_ids'] : '',
				...
				if ( 'new' === $appointment_order ) {
					...
					do_action( 'woocommerce_appointments_create_appointment_page_add_order_item', $order_id );
				}
				...
				$new_appointment->set_status( $create_order ? 'unpaid' : 'confirmed' );
				$new_appointment = apply_filters( 'wcfm_manual_appointment_props', $new_appointment );
				$new_appointment->save();

				?>
				<script>
				  window.location = '<?php echo get_wcfm_view_appointment_url( $new_appointment->get_id() ); ?>';
				</script>
				<?php
				exit;
