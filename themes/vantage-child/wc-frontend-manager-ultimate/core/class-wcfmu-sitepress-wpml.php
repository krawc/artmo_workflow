...
class WCFMu_Sitepress_WPML extends WPML_WPDB_And_SP_User {
	...
	public function __construct() {
		...
		if ( defined( 'ICL_SITEPRESS_VERSION' ) && ! ICL_PLUGIN_INACTIVE && class_exists( 'SitePress' ) ) {
			...
			add_action( 'wcfm_product_manager_gallery_fields_end', array( &$this, 'wpml_wcfm_product_manager_translations' ) );
		}
	}
  ...
  public function wpml_wcfm_load_scripts( $end_point ) {
	  ...
	  switch( $end_point ) {
	  	case 'wcfm-products-manage':
	  		wp_enqueue_script( 'wcfm_wpml_products_manage_js', $WCFMu->library->js_lib_url . 'thirdparty/wcfm-script-wpml-products-manage.js', array( 'jquery', 'wcfm_products_manage_js' ), $WCFMu->version, true );
	  	break;
	  }
	}
				...
				if ( count( $active_languages ) > 0 ) {
					foreach ( $active_languages as $language_data ) {
						...
						if( $translated_id ) {
							...
							$translation_edit_url = '<a href="' . get_wcfm_edit_product_url( $translated_id ) . '" title="' . $translate_text . '"><img style="padding:1px;margin:2px;" border="0" src="' . ICL_PLUGIN_URL . '/res/img/edit_translation.png" alt="' . $translate_text . '" width="16" height="16" /></a>';
						} else {
			...
			if( $product_id ) {
				if( isset( $_POST['lang'] ) && !empty( $_POST['lang'] ) ) {
					...
					if( $lang_code ) {
						...
						$trid_elements             = $sitepress->get_element_translations( $product_id, 'post_product' );
						if($trid_elements) {
