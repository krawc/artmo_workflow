...
class WCFMu_Bulk_Edit {
	...
	public function load_scripts( $end_point ) {
	  ...
	  switch( $end_point ) {
	    ...
	    case 'wcfm-products':
	    	wp_enqueue_script( 'wcfmu_products_bulk_edit_js', $WCFMu->library->js_lib_url . 'wcfmu-script-products-bulk-edit.js', array('jquery'), $WCFMu->version, true );
	    break;
	  }
	}
	...
	public function load_styles( $end_point ) {
	  ...
	  switch( $end_point ) {
	    ...
	    case 'wcfm-products':
	    	wp_enqueue_style( 'wcfmu_products_bulk_edit_css',  $WCFMu->library->css_lib_url . 'wcfmu-style-products-bulk-edit.css', array(), $WCFMu->version );
		  break;
		}
	}
	...
	function wcfmu_bulk_edit_html() {
		...
		require_once( $WCFMu->library->views_path . 'wcfmu-view-products-bulk-manage.php' );
		die;
	}
  	...
  	if( isset( $_POST['controller'] ) ) {
  		...
  		switch( $controller ) {
  			case 'wcfm-products-bulk-manage':
  				require_once( $controllers_path . 'wcfmu-controller-products-bulk-manage.php' );
