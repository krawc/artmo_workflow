...
//		$this->plugin_url = trailingslashit(plugins_url('', $plugin = $file));
		$this->plugin_url = get_stylesheet_directory_uri() . '/wc-frontend-manager-ultimate/';
		...
		add_filter( 'wcfm_modules',  array( &$this, 'get_wcfmu_modules' ) );
	}
	...
	function init() {
		...
		if( $is_allow_products_addons = apply_filters( 'wcfm_is_allow_products_addons', true ) ) {
		...
		if( $is_allow_wc_accommodation = apply_filters( 'wcfm_is_allow_wc_accommodation', true ) ) {
		...
		// Init Bulk Edit
		if( $wcfm_is_allow_bulk_edit = apply_filters( 'wcfm_is_allow_bulk_edit', true ) ) {
			if ( !is_admin() || defined('DOING_AJAX') ) {
