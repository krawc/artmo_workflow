	...
	public function processing() {
		...
		if(!empty($wcfm_bookings_resources_array)) {
			foreach($wcfm_bookings_resources_array as $wcfm_bookings_resources_single) {
				...
				// Action
				$actions = apply_filters ( 'wcfm_bookings_resources_actions', '<a class="wcfm-action-icon" href="' . get_wcfm_bookings_resources_manage_url( $wcfm_bookings_resources_single->ID ) . '"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Manage Resource', 'wc-frontend-manager-ultimate' ) . '"></span></a>', $wcfm_bookings_resources_single );
