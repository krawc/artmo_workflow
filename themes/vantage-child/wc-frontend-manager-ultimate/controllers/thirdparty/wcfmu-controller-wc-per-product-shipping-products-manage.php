...
class WCFMu_WC_Per_Product_Shipping_Products_Manage_Controller {
	...
	function wcfmu_wc_per_product_shipping_product_variation_save( $wcfm_variation_data, $new_product_id, $variation_id, $variations, $wcfm_products_manage_form_data ) {
		...
		if( isset( $variations['_per_product_shipping_country'] ) ) {
	
			// Enabled or Disabled.
			if( WCFMu_Dependencies::wcfm_wc_per_peroduct_shipping_active_check() ) {
				if ( ! empty( $variations['_per_product_shipping'] ) ) {
			...
			$per_product_shipping_rule_id = ( isset( $variations['_per_product_shipping_item_id'] ) ) ? $variations['_per_product_shipping_item_id'] : 0;
			$rule_order = 0;
			if( !$per_product_shipping_rule_id ) {
				if( !empty($variations['_per_product_shipping_country']) || !empty($variations['_per_product_shipping_state']) || !empty($variations['_per_product_shipping_postcode']) || !empty($variations['_per_product_shipping_cost']) || !empty($variations['_per_product_shipping_item_cost']) ) {
					$wpdb->insert(
						$wpdb->prefix . $this->per_product_shipping_table,
						array(
							'rule_country'   => esc_attr( $variations['_per_product_shipping_country'] ),
							'rule_state'     => esc_attr( $variations['_per_product_shipping_state'] ),
							'rule_postcode'  => esc_attr( $variations['_per_product_shipping_postcode'] ),
							'rule_cost'      => esc_attr( $variations['_per_product_shipping_cost'] ),
							'rule_item_cost' => esc_attr( $variations['_per_product_shipping_item_cost'] ),
							'rule_order'     => $rule_order++,
