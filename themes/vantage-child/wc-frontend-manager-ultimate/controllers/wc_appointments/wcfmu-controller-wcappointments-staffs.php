	...
	public function processing() {
		...
		if(!empty($wcfm_appointments_staffs_array)) {
			foreach( $wcfm_appointments_staffs_array as $wcfm_appointments_staffs_single ) {
				...
				// Action
				$actions = apply_filters ( 'wcfm_appointments_staffs_actions', '<a class="wcfm-action-icon" href="' . get_wcfm_appointments_staffs_manage_url( $wcfm_appointments_staffs_single->ID ) . '"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Manage Staff', 'wc-frontend-manager-ultimate' ) . '"></span></a>', $wcfm_appointments_staffs_single );
