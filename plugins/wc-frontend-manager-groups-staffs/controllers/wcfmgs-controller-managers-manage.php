				...
				if( !$has_error ) {
					...
					if( !$manager_id ) {
						...
						if( !$is_update ) {
							...
							$message = str_replace( '{user_role}', 'Shop Manager', $message );
					...
					if(!$has_error) { echo '{"status": true, "message": "' . $wcfm_manager_messages['manager_saved'] . '", "redirect": "' . apply_filters( 'wcfm_manager_manage_redirect', get_wcfm_shop_managers_dashboard_url(), $manager_id ) . '"}'; }
