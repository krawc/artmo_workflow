	...
	public function processing() {
		...
		if(!empty($wcfm_groups_array)) {
			...
			foreach($wcfm_groups_array as $wcfm_groups_single) {
				...
				if( $thumbnail ) {
					$wcfm_groups_json_arr[$index][] =  '<a href="' . get_wcfm_groups_manage_url($wcfm_groups_single->ID) . '"><img src="' . $thumbnail . '" width="30" /></a>';
				} else {
				...
				// Vendor Count
				if( $is_marketplace ) {
					$_group_vendors = (array) get_post_meta( $wcfm_groups_single->ID, '_group_vendors', true );
				...
				// Manager Count
				$_group_managers = (array) get_post_meta( $wcfm_groups_single->ID, '_group_managers', true );
				...
				if( apply_filters( 'wcfm_is_allow_groups_archive', true ) ) {
					$actions = '<a class="wcfm-action-icon" target="_blank" href="' . get_permalink($wcfm_groups_single->ID) . '"><span class="fa fa-eye text_tip" data-tip="' . esc_attr__( 'View', 'wc-frontend-manager' ) . '"></span></a>'; 
				}
				$actions .= '<a class="wcfm-action-icon" href="' . get_wcfm_groups_manage_url($wcfm_groups_single->ID) . '"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Edit', 'wc-frontend-manager' ) . '"></span></a>';
				$actions .= '<a class="wcfm_group_delete wcfm-action-icon" href="#" data-groupid="' . $wcfm_groups_single->ID . '"><span class="fa fa-trash-o text_tip" data-tip="' . esc_attr__( 'Delete', 'wc-frontend-manager' ) . '"></span></a>';
