...
$staff_capability_options = (array) apply_filters( 'wcfmgs_user_capability', get_option( 'wcfm_shop_staff_capability_options' ), $staff_id );
	  ...
	  <div class="wcfm-container wcfm-top-element-container">
	    <h2><?php if( $staff_id ) { _e('Edit Staff', 'wc-frontend-manager-groups-staffs' ); } else { _e('Add Staff', 'wc-frontend-manager-groups-staffs' ); } ?></h2>
			...
			if( $allow_wp_admin_view = apply_filters( 'wcfm_allow_wp_admin_view', true ) ) {
				?>
				<a target="_blank" class="wcfm_wp_admin_view text_tip" href="<?php echo admin_url('user-new.php'); ?>" data-tip="<?php _e( 'WP Admin View', 'wc-frontend-manager-groups-staffs' ); ?>"><span class="fa fa-wordpress"></span></a>
				<?php
			}
			
			echo '<a class="add_new_wcfm_ele_dashboard text_tip" href="'.get_wcfm_shop_staffs_dashboard_url().'" data-tip="' . __('Manage Staffs', 'wc-frontend-manager-groups-staffs') . '"><span class="fa fa-user"></span></a>';
			
			if( $has_new = apply_filters( 'wcfm_add_new_staff_sub_menu', true ) ) {
				echo '<a class="add_new_wcfm_ele_dashboard text_tip" href="'.get_wcfm_shop_staffs_manage_url().'" data-tip="' . __('Add New Staff', 'wc-frontend-manager-groups-staffs') . '"><span class="fa fa-user-plus"></span><span class="text">' . __( 'Add New', 'wc-frontend-manager' ) . '</span></a>';
			}
			?>
		...
		<form id="wcfm_staffs_manage_form" class="wcfm">
		  ...
		  <?php do_action( 'begin_wcfm_staffs_manage_form' ); ?>
						...
						<?php
						  if( $staff_id ) {
							...
							$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_staff_manager_fields_general', array(  
																																						...
																																						"wcfm_vendor" => array( 'label' => __( 'Vendor', 'wc-frontend-manager-groups-staffs' ), 'type' => 'select', 'options' => $vendor_arr, 'class' => 'wcfm-select wcfm_ele ' .$wcfm_vendor_class, 'label_class' => 'wcfm_title ' . $wcfm_vendor_class, 'value' => $wcfm_vendor ),
								...
								if( WCFMu_Dependencies::wcfm_wc_appointments_active_check() ) {
									$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_staff_manager_fields_availability', array(  
												...
												"_wc_appointment_availability"     => array('label' => __('Custom Availability', 'woocommerce-appointments') , 'type' => 'multiinput', 'class' => 'wcfm-text wcfm_ele', 'label_class' => 'wcfm_title', 'desc' => esc_attr( get_wc_appointment_rules_explanation() ), 'desc_class' => 'avail_rules_desc', 'value' => $availability_rule_values, 'options' => array(
																																...
																																"qty" => array('label' => __('Capacity', 'wc-frontend-manager-ultimate'), 'type' => 'number', 'class' => 'wcfm-text wcfm_ele avail_rules_ele avail_rule_capacity avail_rules_text appointment', 'label_class' => 'wcfm_title avail_rules_ele avail_rules_label appointment', 'hints' => __( 'The maximum number of appointments per slot. Overrides general product capacity.', 'woocommerce-appointments' ) ),
			...
			<div class="user_custom_capability" style="display: none;">
			  <?php do_action( 'wcfmgs_capability_manager', $staff_capability_options ); ?>
			</div>
