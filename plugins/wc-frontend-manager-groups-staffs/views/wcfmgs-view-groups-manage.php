	...
	<div class="wcfm-collapse-content">
	  ...
	  <div class="wcfm-container wcfm-top-element-container">
	  	...
	  	<h2><?php if( $group_id ) { _e('Edit Group', 'wc-frontend-manager-groups-staffs' ); } else { _e('Add Group', 'wc-frontend-manager-groups-staffs' ); } ?></h2>
			...
			<?php
			echo '<a id="add_new_group_dashboard" class="add_new_wcfm_ele_dashboard text_tip" href="'.get_wcfm_groups_dashboard_url().'" data-tip="' . __('Groups', 'wc-frontend-manager-groups-staffs') . '"><span class="fa fa-users"></span><span class="text">' . __( 'Groups', 'wc-frontend-manager-groups-staffs') . '</span></a>';
			if( $group_id && apply_filters( 'wcfm_is_allow_groups_archive', true ) ) {
				echo '<a class="add_new_wcfm_ele_dashboard text_tip" target="_blank" href="'.get_permalink( $group_id ).'" data-tip="' . __('Groups Directory', 'wc-frontend-manager-groups-staffs') . '"><span class="fa fa-eye"></span></a>';
			}
			?>
		...
		<form id="wcfm_groups_manage_form" class="wcfm">
			...
			<?php if( $is_marketplace ) { ?>
				...
				<div class="page_collapsible" id="groups_vendors_manage_capability_head">
					...
					<?php _e('Vendors', 'wc-frontend-manager-groups-staffs'); ?>
				</div> 
				<div class="wcfm-container">
					<div id="groups_vendors_manage_capability_expander" class="wcfm-content">
							<?php
								$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'group_manager_fields_vendors', array(  
																																																"group_vendors" => array( 'label' => __( 'Group Vendors', 'wc-frontend-manager-groups-staffs' ), 'type' => 'select', 'options' => $user_arr, 'attributes' => array( 'multiple' => 'multiple', 'style' => 'width: 60%;' ), 'class' => 'wcfm-select wcfm_ele', 'label_class' => 'wcfm_title', 'value' => $group_vendors ),
																																						) ) );
							?>
					</div>
				</div>
			...
			<!-- collapsible -->
				<div class="page_collapsible" id="groups_managers_manage_capability_head">
					<label class="fa fa-user fa-user-secret"></label>
			...
			<?php do_action( 'wcfmgs_capability_manager', $group_capability_options ); ?>
