...
class WCFMvm {
	...
	static function activate_wcfmvm() {
		...
		if ( false == wp_next_scheduled( 'wcfmvm_membership_scheduler' ) ) {
			wp_schedule_event( time(), 'daily', 'wcfmvm_membership_scheduler' );
		}
	...
	static function deactivate_wcfmvm() {
		...
		wp_clear_scheduled_hook( 'wcfmvm_membership_scheduler' );
	...
	function wcfmvm_membership_color_setting_options() {
		...
		$color_options = apply_filters( 'wcfmvm_membership_color_setting_options', array(
																																				 ...
																																				 'wcfmvm_field_button_color' => array( 'label' => __( 'Subscribe Button Color', 'wc-multivendor-membership' ), 'name' => 'wcfmvm_membership_table_button_bg_color_settings', 'default' => '#2a3344', 'element' => '.wcfm_membership_subscribe_button_wrapper input.wcfm_submit_button', 'style' => 'background-color' ),
																																				 'wcfmvm_field_button_hover_color' => array( 'label' => __( 'Subscribe Button Hover Color', 'wc-multivendor-membership' ), 'name' => 'wcfmvm_membership_table_button_bg_hover_color_settings', 'default' => '#00798b', 'element' => '.wcfm_membership_subscribe_button_wrapper input.wcfm_submit_button:hover', 'style' => 'background-color' ),
																																				 'wcfmvm_field_button_bg_color' => array( 'label' => __( 'Subscribe Button Text Color', 'wc-multivendor-membership' ), 'name' => 'wcfmvm_membership_table_button_color_settings', 'default' => '#ffffff', 'element' => '.wcfm_membership_subscribe_button_wrapper input.wcfm_submit_button', 'style' => 'color' ),
																																			) );
		...
		if( $wcfm_membership ) {
			...
			if( $is_marketplace == 'wcmarketplace' ) {
				...
				if( !WC_Product_Vendors_Utils::is_vendor( $member_id ) ) {
					// Creating Vendor Store
					$term = wp_insert_term( get_user_meta($member_id, 'user_display_name', true), WC_PRODUCT_VENDORS_TAXONOMY );
					if ( ! is_wp_error( $term ) ) {
						// Set Vendor commission
						$vendor_data = array();
						...
						// Update user role
						$member_user->add_role('wc_product_vendors_admin_vendor');
						$member_user->add_role('um_artist');
						update_user_meta( $member_id, '_wcpv_active_vendor', $term['term_id'] );


						  $registered = get_user_meta($member_id, 'registered_vendor', true);
				          ...
				          if ( ! is_wp_error( $term ) ) {
				            ...
				            wp_insert_term(
				              ...
				              array(
				                'description' => '',
				                'slug'        => 'artist_'.$member_id
				              )
				            );

				            add_user_meta($member_id, 'registered_vendor', 'true', true);
			...
			if( !$has_error ) {
				...
				if( ( $wcfm_membership != -1 ) && ( $wcfm_membership != '-1' ) ) {
					...
					if( apply_filters( 'wcfm_is_allow_expiry_as_per_next_schedule', false ) ) {
					...
					if( !$wcfm_register_member ) {
						...
						$subscription_welcome_email_subject = get_option( 'wcfm_membership_switch_notication_subject', '{site_name}: Membership Subscription Successfully Changed' );
						$subscription_welcome_email_content = get_option( 'wcfm_membership_switch_notication_content', '' );
						if( !$subscription_welcome_email_content ) {
							$subscription_welcome_email_content = "Hi {first_name},
																						<br /><br />
					...
					if( !$wcfm_register_member ) {
						...
						$subscription_admin_notication_subject = get_option( 'wcfm_membership_switch_admin_notication_subject', '{site_name}: Vendor Membership Subscription Change' );
						$subscription_admin_notication_content = get_option( 'wcfm_membership_switch_admin_notication_content', '' );
						if( !$subscription_admin_notication_content ) {
							$subscription_admin_notication_content = "Dear Admin,
																												<br /><br />
					...
					wp_mail( apply_filters( 'wcfm_admin_email_membership', get_bloginfo( 'admin_email' ) ), $subject, $message );
					...
					// Vendor Notification
					$registration_email_subject = __( '{site_name}: Successfully Registered', 'wc-multivendor-membership' );
					//$subscription_welcome_email_content = get_option( 'wcfm_membership_subscription_welcome_email_content', '' );
					$registration_email_content = "Hi {first_name},
																						<br /><br />
																						You have successfully registered as vendor to our site. 
																						<br /><br />
																						Your account already setup and ready to configure.
																						<br /><br />
																						Kindly follow the below the link to visit your dashboard.
				...
				if( $free_expiry_reminder || ( ( $is_free == 'no' ) && ( $subscription_type != 'one_time' ) ) ) {
					...
					if( $next_schedule ) {
						...
						if( !$renewal_reminder ) {
							...
							$reminder_notication_subject = get_option( 'wcfm_membership_next_payment_notication_subject', '{site_name}: Membership Subscription Recurring Next Payment' );
							$reminder_notication_content = get_option( 'wcfm_membership_next_payment_notication_content', '' );
						...
						wcfm_log( "Membership Expiry Scheduler :: " . $member->ID . " <=> " . $interval );
