...
if( isset( $wp->query_vars['wcfm-memberships-manage'] ) && !empty( $wp->query_vars['wcfm-memberships-manage'] ) ) {
    ...
    $related_subscriptions = get_post_meta( $membership_id, 'related_subscriptions', true );
    $is_subscription_variation = get_post_meta( $membership_id, 'is_subscription_variation', true ) ? get_post_meta( $membership_id, 'is_subscription_variation', true ) : 'no';
	...
	<div class="wcfm-collapse-content">
	  ...
	  <div class="wcfm-container wcfm-top-element-container">
	   <h2><?php if( $membership_id ) { _e('Edit Membership', 'wc-multivendor-membership' ); } else { _e('Add Membership', 'wc-multivendor-membership' ); } ?></h2>
			...
			<?php
			echo '<a class="wcfm_gloabl_settings text_tip" href="'.get_wcfm_memberships_settings_url().'" data-tip="' . __('Settings', 'wc-frontend-manager') . '"><span class="fa fa-cog"></span></a>';
			echo '<a id="add_new_Membership_dashboard" class="add_new_wcfm_ele_dashboard text_tip" href="'.get_wcfm_memberships_url().'" data-tip="' . __('Memberships', 'wc-multivendor-membership') . '"><span class="fa fa-user-plus"></span><span class="text">' . __( 'Memberships', 'wc-multivendor-membership') . '</span></a>';
		...
		<form id="wcfm_memberships_manage_form" class="wcfm">
			...
			<div class="wcfm-tabWrap">
				...
				<div class="page_collapsible" id="memberships_groups_manage_capability_head">
					<label class="fa fa-user fa-user-times"></label>
					...
					<div id="memberships_groups_manage_capability_expander" class="wcfm-content">
					  ...
					  if( WCFM_Dependencies::wcfmgs_plugin_active_check() ) {
							$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'membership_manager_fields_groups', array(
                                                                          ...
                                                                          "related_subscriptions" => array( 'label' => __( 'Related Subscriptions', 'wc-multivendor-membership' ), 'type' => 'text', 'class' => 'wcfm-text wcfm_ele', 'label_class' => 'wcfm_title', 'value' => $related_subscriptions ),
                                                                          "is_subscription_variation" => array( 'label' => __( 'Is Subscription Variation?', 'wc-multivendor-membership' ), 'type' => 'checkbox', 'class' => 'wcfm-checkbox wcfm_ele', 'label_class' => 'wcfm_title checkbox_title', 'value' => 'yes', 'dfvalue' => $is_subscription_variation, 'hints' => __( 'Whether user required Admin Approval to become vendor or not!', 'wc-multivendor-membership' ) ),
                                                                          ) ) );
						} else {
