	...
	public function wcfmu_plugin_row_meta( $links, $file ) {
		...
		if ( $WCFMu->plugin_base_name == $file ) {
			$row_meta = array(
				...
				//'support'      => '<a target="_blank" href="' . esc_url( apply_filters( 'wcfm_support_url', 'http://wclovers.com/forums' ) ) . '" aria-label="' . esc_attr__( 'Visit premium customer support', 'woocommerce' ) . '">' . esc_html__( 'Support', 'woocommerce' ) . '</a>',
