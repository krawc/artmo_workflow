...
class WCFMu_Support {
  ...
  public function load_scripts( $end_point ) {
	  ...
	  switch( $end_point ) {
      	...
      	$wcfm_screen_manager_data = array();
	    	...
	    	$wcfm_screen_manager_data = apply_filters( 'wcfm_support_screen_manage', $wcfm_screen_manager_data );
  ...
  function wcfm_support_order_action( $actions, $order ) {
  	...
  	$order_status = sanitize_title( $order->get_status() );
		if( in_array( $order_status, apply_filters( 'wcfm_support_disable_order_status', array( 'failed', 'cancelled', 'refunded', 'pending', 'on-hold' ) ) ) ) return $actions;
  ...
  function delete_wcfm_support() {
  	...
  	if( isset( $_POST['supportid'] ) && !empty( $_POST['supportid'] ) ) {
  		...
  		$wpdb->query( "DELETE FROM {$wpdb->prefix}wcfm_enquiries WHERE ID = {$supportid}" );
  	}
	...
	function wcfm_dashboard_support_list() {
		...
		if( apply_filters( 'wcfm_is_pref_support', true ) && apply_filters( 'wcfm_is_allow_support', true ) ) {
			...
			<div class="wcfm_dashboard_enquiries">
				<div class="page_collapsible" id="wcfm_dashboard_enquiries"><span class="fa fa-question-circle fa-question-circle-o"></span><span class="dashboard_widget_head"><?php _e('Enquiries', 'wc-frontend-manager-ultimate'); ?></span></div>
					  ...
					  if( !empty( $wcfm_supports_array ) ) {
					  	$counter = 0;
							foreach($wcfm_supports_array as $wcfm_supports_single) {
								if( $counter == 6 ) break;
								echo '<div class="wcfm_dashboard_support"><a href="' . get_wcfm_support_manage_url($wcfm_supports_single->ID) . '" class="wcfm_dashboard_item_title"><span class="fa fa-question-circle-o"></span>' . substr( $wcfm_supports_single->support, 0, 80 ) . ' ...</a></div>';
								$counter++;
							}
