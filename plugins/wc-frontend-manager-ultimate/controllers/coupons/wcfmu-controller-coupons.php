...
class WCFMu_Coupons_Controller {
	...
	public function wcfmu_coupons_actions( $actions, $wcfm_coupons_single ) {
  	...
  	if( $wcfm_coupons_single->post_status == 'publish' ) {
  	  $actions .= ( current_user_can( 'delete_published_shop_coupons' ) ) ? '<a class="wcfm_coupon_delete wcfm-action-icon" href="#" data-couponid="' . $wcfm_coupons_single->ID . '"><span class="fa fa-trash-o text_tip" data-tip="' . esc_attr__( 'Delete', 'wc-frontend-manager-ultimate' ) . '"></span></a>' : '';
  	} else {
  		$actions .= ( current_user_can( 'delete_shop_coupons' ) ) ? '<a class="wcfm_coupon_delete wcfm-action-icon" href="#" data-couponid="' . $wcfm_coupons_single->ID . '"><span class="fa fa-trash-o text_tip" data-tip="' . esc_attr__( 'Delete', 'wc-frontend-manager-ultimate' ) . '"></span></a>' : '';
  	}
