 ...
 */
 	...
 	public function processing() {
   	...
   	if( isset($_POST['product_type']) && !empty($_POST['product_type']) ) {
 		...
 		if( wcfm_is_vendor() ) {
 			...
 			foreach($wcfm_products_counts as $wcfm_products_type => $wcfm_products_count ) {
 				if( in_array( $wcfm_products_type, array( 'publish', 'pending', 'draft' ) ) ) {
 		...
 		if(!empty($wcfm_products_array)) {
 			...
 			foreach($wcfm_products_array as $wcfm_products_single) {
 				...
 				if( current_user_can( 'edit_published_products' ) && apply_filters( 'wcfm_is_allow_edit_products', true ) ) {
 					$wcfm_products_json_arr[$index][] =  '<a href="' . get_wcfm_edit_product_url($wcfm_products_single->ID, $the_product) . '">' . $the_product->get_image( 'medium' ) . '</a>';
 				} else {
 					$wcfm_products_json_arr[$index][] =  $the_product->get_image( 'medium' );
 				}
 				// Title
 				if( current_user_can( 'edit_published_products' ) && apply_filters( 'wcfm_is_allow_edit_products', true ) ) {
 ...
 //.product-date
 				//
 				// SKU
        $note_to_self = get_post_field('post_content', $wcfm_products_single->ID);

 				$wcfm_products_json_arr[$index][] =  ( get_post_meta($wcfm_products_single->ID, '_sku', true) ) ? get_post_meta($wcfm_products_single->ID, '_sku', true) : '<span class="wcfm-stock">-</span>';
 				// Status

        $note_self = '';
        if (isset($note_to_self) && !empty($note_to_self)) {
          $note_self = '<div class="note-to-self"><div class="note-icon"><i class="ion ion-android-list"></i></div><div class="note-content">' . $note_to_self .'</div></div>';
        }

 				if( $wcfm_products_single->post_status == 'publish' ) {
 					$wcfm_products_json_arr[$index][] =  '<span class="product-status product-status-' . $wcfm_products_single->post_status . '">' . __( 'Published', 'wc-frontend-manager' ) . '</span>' . $note_self;
 				} else {
 					$wcfm_products_json_arr[$index][] =  '<span class="product-status product-status-' . $wcfm_products_single->post_status . '">' . __( ucfirst( $wcfm_products_single->post_status ), 'wc-frontend-manager' ) . '</span>' . $note_self;
 				}
 				// Stock
 				$stock_status = $the_product->get_stock_quantity();
 				//$stock_options = array('1' => __('In stock', 'wc-frontend-manager'), '0' => __('SOLD', 'wc-frontend-manager') );
 				if ( $stock_status == 0 ) {
 					$stock_html = '<span class="stock-status outofstock">SOLD</span>';
 				} else {
 					$stock_html = '<span class="stock-status instock">' . __( 'In stock', 'woocommerce' ) . '</span>';
 				}
 				...
 				// Date
 				$wcfm_products_json_arr[$index][] =  '<span class="product-date">' . date_i18n( wc_date_format(), strtotime($wcfm_products_single->post_date) ) . '</span>';
 				// Vendor
 				$vendor_name = '';
 				if( !$WCFM->is_marketplace || wcfm_is_vendor() ) {
 					...
 					if( $WCFM->is_marketplace == 'wcmarketplace' ) {
 						...
 						$vendor_name = get_user_meta( $wcfm_products_single->post_author, 'user_display_name', true);
 				...
 				// // Additional Info
 				 $wcfm_products_json_arr[$index][] = apply_filters( 'wcfm_products_additonal_data', '' );
 				$actions = '<div class="wcfm-products-actions">';
 				// Action
 				...
 				if( $wcfm_products_single->post_status == 'publish' ) {
 					$actions .= ( current_user_can( 'edit_published_products' ) && apply_filters( 'wcfm_is_allow_edit_products', true ) ) ? '<a class="wcfm-action-icon wcfmu_product_quick_edit" href="#" data-product="'. $wcfm_products_single->ID . '"><span class="fa fa-link text_tip" data-tip="' . esc_attr__( 'Quick Edit', 'wc-frontend-manager-ultimate' ) . '"></span></a>' : '';
 					...
 					$actions .= ( current_user_can( 'edit_products' ) && apply_filters( 'wcfm_is_allow_edit_products', true ) ) ? '<a class="wcfm-action-icon wcfmu_product_quick_edit" href="#" data-product="'. $wcfm_products_single->ID . '"><span class="fa fa-link text_tip" data-tip="' . esc_attr__( 'Quick Edit', 'wc-frontend-manager-ultimate' ) . '"></span></a>' : '';
 					$actions .= ( current_user_can( 'edit_products' ) && apply_filters( 'wcfm_is_allow_edit_products', true ) ) ? '<a class="wcfm-action-icon" target="_blank" href="' . get_wcfm_edit_product_url($wcfm_products_single->ID, $the_product) . '"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Edit', 'wc-frontend-manager' ) . '"></span></a>' : '';
