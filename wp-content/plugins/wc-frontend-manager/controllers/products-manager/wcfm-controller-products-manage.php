	...
	public function processing() {
	  ...
	  $has_error = false;
		...
		$dataFilled = ((!empty($wcfm_products_manage_form_data['featured_img'])
				...
				&& !(empty($wcfm_products_manage_form_data['regular_price'] ) && empty($wcfm_products_manage_form_data['priceOnRequest']))
			) ? true : false);
		...
		if ($dataFilled || $is_draft) {
	  	...
	  	// Creating new product
			...
			if(!is_wp_error($new_product_id)) {
				...
				//Associate the product with vendor of the produt author i.e. the ARTIST
				if ($WCFM->is_marketplace == 'wcpvendors' ) {
					$vendor_info = WC_Product_Vendors_Utils::get_all_vendor_data( $author_id );
					$author_vendor_id = key( $vendor_info );
					if ( ! empty( $author_vendor_id ) ) {
						$wcfm_products_manage_form_data['wcfm_associate_vendor'] = $author_vendor_id;
					}
				}
							...
							if( isset($wcfm_products_manage_form_data['gallery_img']) && !empty($wcfm_products_manage_form_data['gallery_img']) ) {
								...
								foreach($wcfm_products_manage_form_data['gallery_img'] as $gallery_imgs) {
									if(isset($gallery_imgs['image']) && !empty($gallery_imgs['image'])) {
										...
										wp_set_object_terms( $new_product_id, 'contemporary', 'genre_tag', true );
