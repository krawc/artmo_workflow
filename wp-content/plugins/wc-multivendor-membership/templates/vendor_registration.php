...
$classes = (($user_id) ? ' wcfmvm-logged-in' : ' wcfmvm-logged-out');
?>

<div id="wcfm_membership_container">
	<div class="wcfm_membership_terms_conditions">
		<?php
		$the_slug = 'terms-and-conditions';
		$args = array(
		  'name'        => $the_slug,
		  'post_type'   => 'page',
		);
  ...
  <form id="wcfm_membership_registration_form" class="<?php echo 'wcfm' . $classes?>">
				...
				do_action( 'begin_wcfm_membership_registration_form' );
				 if( $user_id ) {
					...
					$registration_name_fields = apply_filters( 'wcfm_membership_registration_fields', array(
																																									...
																																									"store_name" => array( 'label' => __('Store Name', 'wc-multivendor-membership') , 'type' => 'text', 'custom_attributes' => array( 'required' => 1 ), 'class' => 'wcfm-text wcfm_ele ', 'label_class' => 'wcfm_ele wcfm_title', 'value' => uniqid() ),
																																	   ) );
	  ...
	  <?php } elseif ( class_exists( 'anr_captcha_class' ) && function_exists( 'anr_captcha_form_field' ) ) { ?>
		...
		<div class="wcfm-membership-buttons">
			<div id="wcfm_membership_registration_submit" class="wcfm_form_simple_submit_wrapper">
			  <?php if( wcfm_is_allowed_membership() ) { ?>
					...
					<a href="<?php echo apply_filters( 'wcfm_change_membership_url', get_wcfm_membership_url() ); ?>" class="wcfm_submit_button"><i class="ion ion-chevron-left"></i> <?php _e( 'Plans', 'wc-multivendor-membership' ); ?></a>
					<?php } ?>
				  <button type="submit" name="save-data" value="<?php if( $user_id ) { _e( 'Confirm', 'wc-multivendor-membership' ); } else { _e( 'Register', 'wc-multivendor-membership' ); } ?>" id="wcfm_membership_register_button" class="wcfm_submit_button" /><?php if( $user_id ) { _e( 'Confirm', 'wc-multivendor-membership' ); } else { _e( 'Register', 'wc-multivendor-membership' ); } ?> <i class="ion ion-chevron-right"></i></button>
				<?php } else { ?>
