	  ...
	  if(isset($wcfm_membership_manager_form_data['title']) && !empty($wcfm_membership_manager_form_data['title'])) {
	  	...
	  	// Creating new membership
			...
			if(!is_wp_error($new_membership_id)) {
				...
				if( isset( $wcfm_membership_manager_form_data['related_subscriptions'] ) ) {
					update_post_meta( $new_membership_id, 'related_subscriptions', $wcfm_membership_manager_form_data['related_subscriptions'] );
				} else {
					update_post_meta( $new_membership_id, 'related_subscriptions', '' );
				}

				// Memvership Subscription Required Admin Approval
				if( isset( $wcfm_membership_manager_form_data['is_subscription_variation'] ) ) {
					update_post_meta( $new_membership_id, 'is_subscription_variation', 'yes' );
				} else {
					update_post_meta( $new_membership_id, 'is_subscription_variation', 'no' );
				}
