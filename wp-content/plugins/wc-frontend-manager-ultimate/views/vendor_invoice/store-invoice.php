<?php 
global $WCFM, $WCFMu, $wpdb, $wpo_wcpdf, $order, $order_id, $document, $document_type, $vendor_id, $process_item_ids, $process_product_ids, $process_sipping_items, $wcfm_vendor_invoice_id; 
...
if( apply_filters( 'wcfm_order_details_tax_line_item', true ) ) {
	if ( wc_tax_enabled() ) {
...
?>
<?php do_action( 'wcfm_before_store_invoice_document', $vendor_id, $order_id, $order ); ?>
		...
		<td class="shop-info">
		  <?php if( $wcfm_vendor_invoice_store ) { ?>
			...
			<?php if( $wcfm_vendor_invoice_address ) { ?>
				<div class="vendor-shop-address"><?php echo $WCFM->wcfm_vendor_support->wcfm_get_vendor_address_by_vendor( $vendor_id ); ?></div>
		  <?php } ?>
		  <?php if( $wcfm_vendor_invoice_email ) { ?>
		  	<div class="vendor-shop-email"><?php echo $WCFM->wcfm_vendor_support->wcfm_get_vendor_email_by_vendor( $vendor_id ); ?></div>
		  <?php } ?>
		  <?php if( $wcfm_vendor_invoice_phone ) { ?>
		  	<div class="vendor-shop-phone"><?php echo $WCFM->wcfm_vendor_support->wcfm_get_vendor_phone_by_vendor( $vendor_id ); ?></div>
		  <?php } ?>
		</td>
	</tr>
</table>
...
<?php if( $document->get_header_logo_id() ) echo apply_filters( 'wcfm_store_invoice_title', __( 'Store Invoice', 'wc-frontend-manager-ultimate' ) ); ?>
</h1>

<?php do_action( 'wcfm_after_store_invoice_document_label', $vendor_id, $order_id, $order ); ?>
					...
					foreach( $wcfm_vendor_invoice_fields as $wvif_key => $wcfm_vendor_invoice_field ) {
						if( isset($wcfm_vendor_invoice_field['is_active']) && $wcfm_vendor_invoice_field['field'] ) {
							if( isset( $wcfm_vendor_invoice_data[$wvif_key] ) && $wcfm_vendor_invoice_data[$wvif_key] ) {
								?>
								<tr class="payment-method">
									<th><?php _e( $wcfm_vendor_invoice_field['field'], 'wc-frontend-manager-ultimate' ); ?>:</th>
									<td><?php echo $wcfm_vendor_invoice_data[$wvif_key]; ?></td>
								</tr>
								<?php
							}
						}
...
<table class="order-details">
	...
	<?php if( !empty( $process_sipping_items ) ) { ?>
		<tbody id="order_shipping_line_items">
		  <?php
			$shipping_methods = WC()->shipping() ? WC()->shipping->load_shipping_methods() : array();
			foreach ( $process_sipping_items as $item_id => $item ) {
				?>
				<tr class="shipping <?php echo ( ! empty( $class ) ) ? $class : ''; ?>" data-order_item_id="<?php echo $item_id; ?>">
					...
					<?php
						if ( ( $tax_data = $item->get_taxes() ) && wc_tax_enabled() && apply_filters( 'wcfm_order_details_tax_line_item', true ) ) {
							foreach ( $order_taxes as $tax_item ) {
				...
				<table class="totals">
					<tfoot>
						...
						<?php if ( wc_tax_enabled() ) : ?>
							<tr>
								<td class="label description" colspan="2" style="text-align:right;"><?php _e( 'Tax', 'wc-frontend-manager-ultimate' ); ?>:</td>
						...
						<tr>
							<td class="label description" colspan="2" style="text-align:right;"><?php _e( 'Shipping', 'wc-frontend-manager-ultimate' ); ?>:</td>
						...
						<tr>
							<td class="label description" colspan="2" style="text-align:right;"><?php _e( 'Shipping Tax', 'wc-frontend-manager-ultimate' ); ?>:</td>
							<td class="total price" style="text-align:center;">
							  <?php
							  if( wcfm_is_marketplace() == 'wcfmmarketplace' ) {
						...
						<tr>
							<td class="label description" colspan="2" style="text-align:right;"><?php _e( 'Total', 'wc-frontend-manager-ultimate' ); ?>:</td>
							<td class="total price" style="text-align:center;">
...
<?php
if( apply_filters( 'wcfm_is_pref_policies', true ) && $WCFM->wcfm_vendor_support->wcfm_vendor_has_capability( $vendor_id, 'policy' ) && $wcfm_vendor_invoice_policies ) {
...
if( $wcfm_vendor_invoice_disclaimer && $wcfm_vendor_disclaimer ) {
	...
	<table width="100%" style="width:100%;">
	  <tbody>
			<tr>
				<th></th>
				<td colspan="7"><b><?php echo $wcfm_vendor_disclaimer; ?></b></td>
			</tr>
		</tbody>
	</table>
	<br/><br/>
...
if( $wcfm_vendor_invoice_signature && $wcfm_vendor_signature ) {
	...
	<table width="100%" style="width:100%;">
	  <tbody>
			<tr>
				<td><div style="width:100%;text-align: right;"><img src="<?php echo $wcfm_vendor_signature; ?>" /></div></td>
			</tr>
		</tbody>
	</table>
	<br/><br/>
