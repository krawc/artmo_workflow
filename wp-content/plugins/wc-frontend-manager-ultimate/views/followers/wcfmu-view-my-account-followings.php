...
if( is_user_logged_in() ) {
	...
	<table class="woocommerce-followings-table woocommerce-MyAccount-followings shop_table shop_table_responsive my_account_followings account-followings-table">
		<thead>
			<tr>
				<th class="woocommerce-followings-tickets-table__header woocommerce-followings-tickets-table__header-followings-name"><span class="nobr"><?php _e( 'Store', 'wc-frontend-manager-ultimate' ); ?></span></th>
		...
		<tbody>
			<?php if( !empty( $wcfm_followings ) ) { foreach( $wcfm_followings as $wcfm_following_single ) { ?>
				<tr class="woocommerce-followings-table__row woocommerce-followings-table__row--status-completed followings">
					<td class="woocommerce-followings-table__cell woocommerce-followings-table__cell-followings-name" data-title="Followings Name">
						<?php echo $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_by_vendor( $wcfm_following_single->user_id ); ?>
					</td>
					<?php if( apply_filters( 'wcfm_is_allow_followings_email', true ) ) { ?>
						<td class="woocommerce-followings-table__cell woocommerce-followings-table__cell-followings-email" data-title="Followings Email">
							<a href="mailto:<?php echo $wcfm_following_single->user_email; ?>"><?php echo $wcfm_following_single->user_email; ?></a>
						</td>
					<?php } ?>
					<td class="woocommerce-followings-table__cell woocommerce-followings-table__cell-followings-actions" data-title="Actions">
						<a class="wcfm_followings_delete wcfm-action-icon" href="#" data-lineid="<?php echo $wcfm_following_single->ID; ?>" data-followersid="<?php echo $wcfm_following_single->follower_id; ?>" data-userid="<?php echo $wcfm_following_single->user_id; ?>" class="woocommerce-button button delete"><span class="fa fa-trash-o text_tip" data-tip="<?php echo esc_attr__( 'Delete', 'wc-frontend-manager-ultimate' ); ?>"></span></a>													
					</td>
				</tr>
			<?php } } else { ?>
				<tr class="woocommerce-followings-table__row woocommerce-followings-table__row--status-completed followings">
				  <td class="woocommerce-followings-table__cell woocommerce-followings-table__cell-followings-no" data-title="NO Followings" colspan="2">
