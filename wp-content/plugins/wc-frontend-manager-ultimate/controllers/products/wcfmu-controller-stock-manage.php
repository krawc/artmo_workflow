  	...
  	if( isset($_POST['product_type']) && !empty($_POST['product_type']) ) {
		...
		if(!empty($wcfm_products_array)) {
			...
			foreach($wcfm_products_array as $wcfm_products_single) {
				...
				if( current_user_can( 'edit_published_products' ) && apply_filters( 'wcfm_is_allow_edit_products', true ) && apply_filters( 'wcfm_is_allow_edit_specific_products', true, $product_edit_id ) ) {
				...
				$wcfm_products_json_arr[$index][] =  '<select class="wcfm-select" name="stock_manage['.$wcfm_products_single->ID.'][manage_stock]">' . $manage_stock_options . '</select>';
