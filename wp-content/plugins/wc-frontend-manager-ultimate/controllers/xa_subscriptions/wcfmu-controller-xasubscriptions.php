	...
	public function processing() {
		...
		if(!empty($wcfm_subscriptions_array)) {
			...
			foreach($wcfm_subscriptions_array as $wcfm_subscriptions_single) {
				...
				$actions = '&ndash;';
				if( apply_filters( 'wcfm_is_allow_subscription_details', true ) ) {
					$actions = apply_filters ( 'wcfm_subscriptions_actions', '<a class="wcfm-action-icon" href="' . get_wcfm_subscriptions_manage_url( $wcfm_subscriptions_single->ID, $the_subscription ) . '"><span class="fa fa-eye text_tip" data-tip="' . esc_attr__( 'View Details', 'wc-frontend-manager' ) . '"></span></a>', $wcfm_subscriptions_single, $the_subscription );
				}
