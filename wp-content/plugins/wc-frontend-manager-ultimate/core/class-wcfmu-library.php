...
class WCFMu_Library {
  ...
  public $views_path;
    ...
    add_action( 'end_wcfm_products_manage', array( &$this, 'end_wcfm_products_manage_form_load_views' ), 489 );
	...
	public function load_styles( $end_point ) {
	  ...
	  switch( $end_point ) {
    	...
    	break;
		  ...
		  case 'wcfm-reports-sales-by-product':
		  	wp_enqueue_style( 'reports_menus_css',  $WCFM->library->css_lib_url . 'reports/wcfm-style-reports-menus.css', array(), $WCFMu->version );
	...
	public function load_views( $end_point ) {
      ...
      case 'wcfm-stock-manage':
    		include_once( $this->views_path . 'products/wcfmu-view-stock-manage.php' );
    	break;
      
      case 'wcfm-products-import':
        include_once( $this->views_path . 'products/wcfmu-view-products-import.php' );
      break;
	...
	*/
	...
	public function load_wcfm_login_popup_lib() {
		global $WCFMu;
	  wp_enqueue_script( 'wcfm_login_js', $WCFMu->plugin_url . 'includes/libs/login-popup/wcfm-login.js', array('jquery'), $WCFMu->version, true );
	  wp_enqueue_style( 'wcfm_login_css',  $WCFMu->plugin_url . 'includes/libs/login-popup/wcfm-login.css', array(), $WCFMu->version );
	  
	  wp_localize_script( 'wcfm_login_js', 'wcfm_login_messages', array( 'no_username' => __( 'Please insert username before submit.', 'wc-frontend-manager-ultimate' ), 'no_password' => __( 'Please insert password before submit.', 'wc-frontend-manager-ultimate' )) );
	}
