...
class WCFMu_WCAppointments {
  ...
  function wcfm_appointment_gcal_user_profile() {
  	global $WCFM, $WCFMvm;
		...
		?>
		<div class="page_collapsible wcfm_profile_manage_apt_gcal_sync" id="wcfm_profile_manage_form_apt_gcal_sync_head"><label class="fa fa-calendar"></label><?php _e( 'Google Calendar Sync', 'wc-frontend-manager-ultimate' ); ?><span></span></div>
		<div class="wcfm-container">
			<div id="wcfm_profile_manage_form_apt_gcal_sync_expander" class="wcfm-content">
			  <table>
			    <tr>
						<th><label><?php _e( 'Authorization', 'woocommerce-appointments' ); ?></label></th>
						<td>
							...
							<?php else : ?>
								<p><?php printf( __( 'Please configure <a href="%s">Google Calendar Sync settings</a> first.', 'woocommerce-appointments' ), admin_url( 'admin.php?page=wc-settings&tab=appointments&section=gcal' ) ); ?>
							<?php endif; ?>
						</td>
					</tr>
					<?php if ( $access_token ) : ?>
						<tr>
							<th><label for="wc_appointments_gcal_calendar_id"><?php _e( 'Calendar ID', 'woocommerce-appointments' ); ?></label></th>
							<td>
								<?php $calendar_id = ( $calendar_id = get_user_meta( $user_id, 'wc_appointments_gcal_calendar_id', true ) ) ? $calendar_id : ''; ?>
								<?php if ( $calendar_id ) : ?>
									<input type="text" class="regular-text" name="wc_appointments_gcal_calendar_id" id="wc_appointments_gcal_calendar_id" value="<?php echo $calendar_id; ?>">
								<?php else : ?>
									<select id="wc_appointments_gcal_calendar_id" name="wc_appointments_gcal_calendar_id" class="wc-enhanced-select" style="width:25em;">
										...
										if ( $access_token && $get_user_calendars ) {
											...
											?>
												<option value="<?php echo $cal_id; ?>" <?php selected( $calendar_id, $cal_id ); ?>><?php echo $cal_name; ?></option>
											<?php
											}
										}
										?>
						...
						if ( $calendar_id && $twoway_enabled ) : ?>
							<tr>
								<th><label><?php _e( 'Last Sync', 'woocommerce-appointments' ); ?></label></th>
								<td>
									<?php
									#$rules = get_user_meta( $user->ID, 'wc_appointments_gcal_availability', true );
		
									#print '<pre>'; print_r( $rules ); print '</pre>';
									$last_synced  = ( $last_synced = get_user_meta( $user_id, 'wc_appointments_gcal_availability_last_synced', true ) ) ? $last_synced : '';
									if ( $last_synced ) {
									...
									?>
										<p class="last_synced"><?php echo $ls_message; ?></p>
									<?php } else { ?>
										<p class="last_synced"><?php _e( 'Not synced yet.', 'woocommerce-appointments' ); ?></p>
									<?php } ?>
									<p class="submit">
										<button type="button" class="button manual_sync" data-staff="<?php echo esc_attr( absint( $user_id ) ); ?>"><?php _e( 'Sync Manually', 'woocommerce-appointments' ); ?></button>
									</p>
								</td>
							</tr>
						<?php endif; ?>
