...
$manager_capability_options = (array) apply_filters( 'wcfmgs_user_capability', get_option( 'wcfm_shop_manager_capability_options' ), $manager_id );
	...
	<div class="wcfm-collapse-content">
	  ...
	  <div class="wcfm-container wcfm-top-element-container">
	    <h2><?php if( $manager_id ) { _e('Edit Manager', 'wc-frontend-manager-groups-staffs' ); } else { _e('Add Manager', 'wc-frontend-manager-groups-staffs' ); } ?></h2>
			...
			echo '<a class="add_new_wcfm_ele_dashboard text_tip" href="'.get_wcfm_shop_managers_dashboard_url().'" data-tip="' . __('Manage Managers', 'wc-frontend-manager-groups-staffs') . '"><span class="fa fa-user-secret"></span></a>';
			
			if( $has_new = apply_filters( 'wcfm_add_new_manager_sub_menu', true ) ) {
				echo '<a class="add_new_wcfm_ele_dashboard text_tip" href="'.get_wcfm_shop_managers_manage_url().'" data-tip="' . __('Add New Manager', 'wc-frontend-manager-groups-staffs') . '"><span class="fa fa-user-plus"></span><span class="text">' . __( 'Add New', 'wc-frontend-manager' ) . '</span></a>';
			}
			?>
		  ...
		  <?php do_action( 'begin_wcfm_managers_manage_form' ); ?>
			...
			<?php if( !wcfm_is_group_manager( $manager_id ) ) { ?>
				<div class="user_custom_capability" style="display: none;">
					<?php do_action( 'wcfmgs_capability_manager', $manager_capability_options ); ?>
				</div>
			<?php } ?>
