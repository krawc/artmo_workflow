	...
	public function processing() {
		...
		if(!empty($wcfm_shop_staffs_array)) {
			foreach( $wcfm_shop_staffs_array as $wcfm_shop_staffs_single ) {
				...
				if( $wcfm_vendors_id ) {
					$wcfm_shop_staffs_json_arr[$index][] =  '<span class="wcfm_vendor_store">' . apply_filters( 'wcfm_vednors_store_name_data', $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_by_vendor( $wcfm_vendors_id ), $wcfm_vendors_id ) . '</span>';
				} else {
				...
				// Action
				$actions = '<a class="wcfm-action-icon" href="' . get_wcfm_shop_staffs_manage_url( $wcfm_shop_staffs_single->ID ) . '"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Manage Staff', 'wc-frontend-manager-ultimate' ) . '"></span></a>';
				$actions .= '<a class="wcfm_staff_delete wcfm-action-icon" href="#" data-staffid="' . $wcfm_shop_staffs_single->ID . '"><span class="fa fa-trash-o text_tip" data-tip="' . esc_attr__( 'Delete', 'wc-frontend-manager' ) . '"></span></a>';
