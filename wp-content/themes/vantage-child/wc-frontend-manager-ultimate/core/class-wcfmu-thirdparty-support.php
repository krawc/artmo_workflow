...
class WCFMu_ThirdParty_Support {
	...
	public function __construct() {
		...
		add_filter( 'wcfm_settings_fields_product_types', array( &$this, 'wcfmcap_product_types' ), 60, 3 );
    ...
    if( $wcfm_allow_listings = apply_filters( 'wcfm_is_allow_listings', true ) ) {
			if ( WCFM_Dependencies::wcfm_wp_job_manager_plugin_active_check() && WCFMu_Dependencies::wcfm_products_listings_active_check() ) {
				...
				if( apply_filters( 'wcfm_is_allow_add_products', true ) ) {
					if( current_user_can( 'edit_products' ) ) {
						...
						add_action( 'submit_job_form_end', array( &$this, 'wcfm_add_listing_form_end' ) );
						add_action( 'wp_enqueue_scripts', array( $this, 'wcfm_add_listing_enqueue_scripts' ) );
						add_action( 'after_wcfm_load_scripts', array( $this, 'wcfm_add_listing_after_wcfm_load_scripts' ) );
						add_filter( 'the_content', array( &$this, 'wcfm_add_listing_page' ) );
					}
				}
			}
		}
    ...
    if( apply_filters( 'wcfm_is_allow_toolset_types', true ) ) {
			if ( WCFMu_Dependencies::wcfm_toolset_types_active_check() ) {
				add_action( 'end_wcfm_settings', array( &$this, 'wcfm_toolset_types_settings' ), 150 );
				add_action( 'end_wcfm_products_manage', array( &$this, 'wcfm_toolset_types_product_manage_fields' ), 150 );
			}
		}
    ...
    if( $wcfm_is_allow_wc_box_office = apply_filters( 'wcfm_is_allow_wc_box_office', true ) ) {
			if( WCFMu_Dependencies::wcfm_wc_box_office_active_check() ) {
				add_filter( 'wcfm_product_manage_fields_general', array( &$this, 'wcfm_wc_box_office_product_manage_fields_general' ), 60, 3 );
  	...
  	if( $wcfm_allow_auction = apply_filters( 'wcfm_is_allow_auction', true ) ) {
			if( WCFMu_Dependencies::wcfm_yith_auction_active_check() || WCFMu_Dependencies::wcfm_wcs_auction_active_check() ) {
				$query_auction_vars = array(
					'wcfm-auctions'        => ! empty( $wcfm_modified_endpoints['wcfm-auctions'] ) ? $wcfm_modified_endpoints['wcfm-auctions'] : 'wcfm-auctions',
				);
		...
		if( $wcfm_allow_rental = apply_filters( 'wcfm_is_allow_rental', true ) ) {
			if( WCFMu_Dependencies::wcfm_wc_rental_pro_active_check() ) {
				$query_rental_vars = array(
					'wcfm-rental-calendar'        => ! empty( $wcfm_modified_endpoints['wcfm-rental-calendar'] ) ? $wcfm_modified_endpoints['wcfm-rental-calendar'] : 'wcfm-rental-calendar',
					'wcfm-rental-quote'           => ! empty( $wcfm_modified_endpoints['wcfm-rental-quote'] ) ? $wcfm_modified_endpoints['wcfm-rental-quote'] : 'wcfm-rental-quote',
					'wcfm-rental-quote-details'   => ! empty( $wcfm_modified_endpoints['wcfm-rental-quote-details'] ) ? $wcfm_modified_endpoints['wcfm-rental-quote-details'] : 'wcfm-rental-quote-details',
				);
		...
		if( $wcfm_allow_rental = apply_filters( 'wcfm_is_allow_rental', true ) ) {
			if( WCFMu_Dependencies::wcfm_wc_rental_pro_active_check() ) {
				$rental_endpoints = array(
															'wcfm-rental-calendar'  		   => 'wcfm-rental-calendar',
															'wcfm-rental-quote'  		       => 'wcfm-rental-quote',
															'wcfm-rental-quote-details'    => 'wcfm-rental-quote-details'
															);
	  		...
	  		// WC Per Product Shipping Support - 2.5.0
				if( apply_filters( 'wcfm_is_allow_shipping', true ) && apply_filters( 'wcfm_is_allow_per_product_shipping', true ) ) {
					if ( WCFMu_Dependencies::wcfm_wc_per_peroduct_shipping_active_check() || ( $WCFMu->is_marketplace == 'wcpvendors' ) ) {
						require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-wc-per-product-shipping-products-manage.php' );
					}
				}
				...
				if( $wcfm_is_allow_gallery = apply_filters( 'wcfm_is_allow_gallery', true ) ) {
					if ( WCFMu_Dependencies::wcfm_wc_variation_gallery_active_check() ) {
						require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-wc-variation-gallery-products-manage.php' );
					}
				}
	  	 break;
	  	 
	  	case 'wcfm-rental-calendar':
        require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-wcrental-calendar.php' );
      break;
      
      case 'wcfm-rental-quote':
        require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-wcrental-quote.php' );
      break;
      
      case 'wcfm-rental-quote-details':
        require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-wcrental-quote-details.php' );
      break;
      
      case 'wcfm-auctions':
        require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-auctions.php' );
      break;
	  }
	}
  ...
  public function wcfmu_thirdparty_ajax_controller() {
  	...
  	if( isset( $_POST['controller'] ) ) {
  		...
  		switch( $controller ) {
  			case 'wcfm-products-manage':
  				require_once( $controllers_path . 'thirdparty/wcfmu-controller-thirdparty-products-manage.php' );
					new WCFMu_ThirdParty_Products_Manage_Controller();
					
					// WC Per Product Shipping - Products Support - 2.5.0
					if( apply_filters( 'wcfm_is_allow_shipping', true ) && apply_filters( 'wcfm_is_allow_per_product_shipping', true ) ) {
						if ( WCFMu_Dependencies::wcfm_wc_per_peroduct_shipping_active_check() || ( $WCFMu->is_marketplace == 'wcpvendors' ) ) {
							require_once( $controllers_path . 'thirdparty/wcfmu-controller-wc-per-product-shipping-products-manage.php' );
					...
					if( $wcfm_allow_toolset_types = apply_filters( 'wcfm_is_allow_toolset_types', true ) ) {
						if ( WCFMu_Dependencies::wcfm_toolset_types_active_check() ) {
							require_once( $controllers_path . 'thirdparty/wcfmu-controller-toolset-types-products-manage.php' );
					...
					if( $wcfm_is_allow_gallery = apply_filters( 'wcfm_is_allow_gallery', true ) ) {
						if ( WCFMu_Dependencies::wcfm_wc_variation_gallery_active_check() ) {
							require_once( $controllers_path . 'thirdparty/wcfmu-controller-wc-variation-gallery-products-manage.php' );
					...
					if( $wcfm_allow_acf_fields = apply_filters( 'wcfm_is_allow_acf_fields', true ) ) {
						if ( WCFMu_Dependencies::wcfm_acf_active_check() ) {
							require_once( $controllers_path . 'thirdparty/wcfmu-controller-acf-products-manage.php' );
					...
					if( $wcfm_allow_acf_fields = apply_filters( 'wcfm_is_allow_acf_fields', true ) ) {
						if ( WCFMu_Dependencies::wcfm_acf_pro_active_check() ) {
							require_once( $controllers_path . 'thirdparty/wcfmu-controller-acf-pro-products-manage.php' );
					...
					if( $wcfm_is_allow_map = apply_filters( 'wcfm_is_allow_mappress', true ) ) {
						if ( WCFMu_Dependencies::wcfm_address_geocoder_active_check() ) {
							require_once( $controllers_path . 'thirdparty/wcfmu-controller-address-geocoder-products-manage.php' );
  			...
  			case 'wcfm-rental-quote':
  				require_once( $controllers_path . 'thirdparty/wcfmu-controller-rental-quote.php' );
					new WCFMu_Rental_Quote_Controller();
  			break;
  			
  			case 'wcfm-auctions':
  				require_once( $controllers_path . 'thirdparty/wcfmu-controller-auctions.php' );
  			...
  			case 'wcfm-customers-manage':
  				// Toolset Types - Products Support - 3.0.1
					if( $wcfm_allow_toolset_types = apply_filters( 'wcfm_is_allow_toolset_types', true ) ) {
						if ( WCFMu_Dependencies::wcfm_toolset_types_active_check() ) {
							require_once( $controllers_path . 'thirdparty/wcfmu-controller-toolset-types-user-profile.php' );
				...
				case 'wcfm-settings':
  				// Toolset Types - Products Support - 3.1.7
					if( $wcfm_allow_toolset_types = apply_filters( 'wcfm_is_allow_toolset_types', true ) ) {
						if ( WCFMu_Dependencies::wcfm_toolset_types_active_check() ) {
							require_once( $controllers_path . 'thirdparty/wcfmu-controller-toolset-types-settings.php' );
  ...
  /**
	...
	function wcfm_order_pdf_invoice() {
  	...
  	$order_id = $_GET['order_id'];
		...
		$template = $WCFMu->plugin_path . 'includes/pdf_invoice/' . $document_type . '.php';
		ob_start();
		...
		$template_wrapper = $WCFMu->plugin_path . 'includes/pdf_invoice/html-document-wrapper.php';
		ob_start();
  ...
  /**
	...
	function wcfm_yithauction_product_manage_fields( $product_id = 0, $product_type ) {
		global $WCFM, $WCFMu;
		require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-yithauctions-product-manage.php' );
	}
  ...
  /**
	...
	function wcfm_wcsauction_product_manage_fields( $product_id = 0, $product_type ) {
		global $WCFM, $WCFMu;
		require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-wcsauctions-product-manage.php' );
	}
  ...
  /**
	...
	function wcfm_wcrental_pro_product_manage_fields( $product_id = 0, $product_type ) {
		global $WCFM, $WCFMu;
		require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-wcrentalpro-product-manage.php' );
	}
  ...
  function wcfm_wpjm_associate_listings_product_manage_fields( ) {
		global $WCFMu;
	  require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-wpjm-associate-listings-products-manage.php' );
	}
	...
	function wcfm_wc_lottery_product_manage_fields( $product_id = 0, $product_type ) {
		global $WCFM, $WCFMu;
		require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-wc-lottery-product-manage.php' );
	}
	...
	function wcfm_add_listing_product_manage_fields( $fields ) {
		...
		if( isset( $fields['company'] ) && isset( $fields['company']['products'] ) ) {
			$products_create_description = sprintf( __( '%s | %s', 'wp-job-manager' ), '<a href="#" id="wcfm-add-product"><i class="fa fa-cube"></i> ' . __( 'Create New Product', 'wc-frontend-manager-ultimate' ) . '</a>',  '<a href="' . get_wcfm_products_url() . '" id="wcfm-manage-products" target="_blank"><i class="fa fa-cubes"></i> ' . __( 'Manage Products', 'wc-frontend-manager-ultimate' ) . '</a>');
	...
	/**
	 * Listings - Associate Products - 3.2.1
	 */
	function wcfm_add_listing_form_end() {
		add_filter( 'wcfm_is_allow_headpanels', '__return_false' );
		add_filter( 'wcfm_is_allow_rich_editor', array( &$this, 'wcfm_add_listing_allow_rich_editor'), 600 );
	}
	
	function wcfm_add_listing_allow_rich_editor() {
		return '';
	}
	...
	function wcfm_add_listing_enqueue_scripts() {
		...
		if( ( $add_listings_page && is_object( $post ) && ( $add_listings_page == $post->ID ) ) || ( $job_dashboard_page && is_object( $post ) && ( $job_dashboard_page == $post->ID ) && isset( $_GET['action'] ) && ( $_GET['action'] == 'edit' ) ) ) {
			...
			wp_enqueue_script( 'wcfm_add_listings_products_manage_js', $WCFMu->library->js_lib_url . 'thirdparty/wcfmu-script-add-listings-products-manage.js', array('jquery', 'wcfm_products_manage_js'), $WCFMu->version, true );
			
			// Localized Script
			//$wcfm_dashboard_messages = get_wcfm_dashboard_messages();
			//wp_localize_script( 'wcfm_add_listings_products_manage_js', 'wcfm_dashboard_messages', $wcfm_dashboard_messages );
			
			// Load Styles
			$WCFM->library->load_styles( 'wcfm-products-manage' );
			wp_enqueue_style( 'wcfm_add_listings_products_manage_css', $WCFMu->library->css_lib_url . 'thirdparty/wcfmu-style-add-listings-products-manage.css', array( ), $WCFMu->version );
	...
	function wcfm_add_listing_after_wcfm_load_scripts() {
		global $WCFM, $WCFMu, $post;
		
		$job_dashboard_page = get_option( 'job_manager_job_dashboard_page_id' );
		$add_listings_page = get_option( 'job_manager_submit_job_form_page_id' );
		if( ( $add_listings_page && is_object( $post ) && ( $add_listings_page == $post->ID ) ) || ( $job_dashboard_page && ( $job_dashboard_page == $post->ID ) && isset( $_GET['action'] ) && ( $_GET['action'] == 'edit' ) ) ) {
			//wp_dequeue_script( 'wcfm_menu_js' );
			wp_dequeue_script( 'tinymce_js' );
			//wp_dequeue_script( 'jquery_tinymce_js' );
			wp_dequeue_script( 'jquery_tinymce_theme_js' );
			wp_dequeue_script( 'jquery_tinymce_avlist_js' );
			wp_dequeue_script( 'jquery_tinymce_anchor_js' );
			wp_dequeue_script( 'jquery_tinymce_autolink_js' );
			wp_dequeue_script( 'jquery_tinymce_autosize_js' );
			wp_dequeue_script( 'jquery_tinymce_fullscreen_js' );
			wp_dequeue_script( 'jquery_tinymce_link_js' );
			wp_dequeue_script( 'jquery_tinymce_list_js' );
			wp_dequeue_script( 'jquery_tinymce_preview_js' );
			wp_dequeue_script( 'jquery_tinymce_media_js' );
			wp_dequeue_script( 'jquery_tinymce_image_js' );
			wp_dequeue_script( 'jquery_tinymce_charmap_js' );
			wp_dequeue_script( 'jquery_tinymce_plugin_js' );
			wp_dequeue_script( 'jquery_tinymce_contextmenu_js' );
			wp_dequeue_script( 'jquery_tinymce_directionally_js' );
			wp_dequeue_script( 'jquery_tinymce_datetime_js' );
			wp_dequeue_script( 'jquery_tinymce_paste_js' );
			wp_dequeue_script( 'jquery_tinymce_searchreplace_js' );
			wp_dequeue_script( 'jquery_tinymce_visual_js' );
			wp_dequeue_script( 'jquery_tinymce_table_js' );
			wp_dequeue_script( 'jquery_tinymce_print_js' );
		}
	}
	...
	function wcfm_add_listing_page( $content ) {
		...
		if( $add_listings_page && ( $add_listings_page == $post->ID ) ) {
			$content .= '[wcfm endpoint="wcfm-products-manage" menu="false"]';
   ...
   */
	function wcfm_toolset_types_settings() {
		global $WCFMu;
	  require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-toolset-types-settings.php' );
	}
  ...
  function wcfm_toolset_types_product_manage_fields( ) {
		global $WCFMu;
	  require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-toolset-types-products-manage.php' );
	}
  ...
  function wcfm_mappress_product_manage_fields( ) {
		global $WCFMu;
		require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-mappress-products-manage.php' );
	}
  ...
  function wcfm_toolset_types_user_profile_fields() {
		global $WCFMu;
	  require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-toolset-types-user-profile.php' );
	}
   ...
   */
	function wcfm_toolset_types_user_profile_fields_view( $user_id ) {
		...
		$current_user_roles = isset( $vendor_user->roles ) ? $vendor_user->roles : array( 'vendor', 'dc_vendor', 'seller' );
		...
		if( !empty( $field_groups )) {
			foreach( $field_groups as $field_group_index => $field_group ) {
				...
				if ( count( $for_users ) != 0 ) {
					if ( !in_array( 'customer', $for_users ) ) {
						continue;
					}
				}
  ...
  function wcfm_toolset_types_taxonomy_fields() {
		global $WCFMu;
	  require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-toolset-types-taxonomy.php' );
	}
	
	/**
   * Toolset Types - Products Manage General options - 3.0.4
   */
  function wcfm_acf_product_manage_fields() {
		global $WCFMu;
	  require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-acf-products-manage.php' );
	}
	
	/**
   * Toolset Types - Products Manage General options - 3.3.7
   */
  function wcfm_acf_pro_product_manage_fields() {
		global $WCFMu;
	  require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-acf-pro-products-manage.php' );
	}
  ...
  function wcfm_address_geocoder_product_manage_fields( ) {
		global $WCFMu;
		require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-address-geocoder-products-manage.php' );
	}
  ...
  function wcfm_wc_box_office_product_manage_fields( ) {
		global $WCFMu;
		require_once( $WCFMu->library->views_path . 'thirdparty/wcfmu-view-wc-box-office-products-manage.php' );
	}
