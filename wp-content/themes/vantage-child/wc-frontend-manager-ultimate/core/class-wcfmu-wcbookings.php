...
class WCFMu_WCBookings {
    ...
    if( wcfm_is_booking() ) {
    	...
    	// WCFM view meta boxes
			...
			if ( current_user_can( 'manage_bookings' ) ) {
				...
				add_filter( 'wcfm_product_manage_fields_general', array( &$this, 'wcb_product_manage_fields_general' ), 10, 3 );
    	...
    	// Booking Mark as Confirmed
			...
			add_action( 'wp_ajax_nopriv_wcfm_booking_mark_confirm', array( &$this, 'wcfm_booking_mark_confirm' ) );
			
			// Booking Status Update
			add_action( 'wp_ajax_wcfm_modify_booking_status', array( &$this, 'wcfm_modify_booking_status' ) );
			add_action( 'wp_ajax_nopriv_wcfm_modify_booking_status', array( &$this, 'wcfm_modify_booking_status' ) );
  ...
  /**
	...
	function wcb_wcfm_endpoints_slug( $endpoints ) {
		...
		$booking_endpoints = array(
													'wcfm-bookings-dashboard'        => 'wcfm-bookings-dashboard',
													'wcfm-bookings'                  => 'wcfm-bookings',
													'wcfm-bookings-resources'        => 'wcfm-bookings-resources',
													'wcfm-bookings-resources-manage' => 'wcfm-bookings-resources-manage',
													'wcfm-bookings-manual'    		   => 'wcfm-bookings-manual',
													'wcfm-bookings-calendar'  		   => 'wcfm-bookings-calendar',
													'wcfm-bookings-details'          => 'wcfm-bookings-details',
													'wcfm-bookings-settings'         => 'wcfm-bookings-settings'
													);
  ...
  public function wcb_load_views( $end_point ) {
	  ...
	  switch( $end_point ) {
      case 'wcfm-bookings-resources':
        require_once( $WCFMu->library->views_path . 'wc_bookings/wcfmu-view-wcbookings-resources.php' );
      break;
      
      case 'wcfm-bookings-resources-manage':
        require_once( $WCFMu->library->views_path . 'wc_bookings/wcfmu-view-wcbookings-resources-manage.php' );
      break;
      
      case 'wcfm-bookings-manual':
        require_once( $WCFMu->library->views_path . 'wc_bookings/wcfmu-view-wcbookings-manual.php' );
      break;
      
      case 'wcfm-bookings-calendar':
        require_once( $WCFMu->library->views_path . 'wc_bookings/wcfmu-view-wcbookings-calendar.php' );
      break;
      
      case 'wcfm-bookings-settings':
        require_once( $WCFMu->library->views_path . 'wc_bookings/wcfmu-view-wcbookings-settings.php' );
      break;
	  }
	}
  ...
  public function wcb_ajax_controller() {
  	...
  	if( isset( $_POST['controller'] ) ) {
  		...
  		switch( $controller ) {
  			case 'wcfm-products-manage':
  				require_once( $controllers_path . 'wcfmu-controller-wcbookings-products-manage.php' );
					new WCFMu_WCBookings_Products_Manage_Controller();
  			break;
  			
				case 'wcfm-bookings-resources':
					require_once( $controllers_path . 'wcfmu-controller-wcbookings-resources.php' );
					new WCFMu_WCBookings_Resources_Controller();
				break;
				
				case 'wcfm-bookings-resources-manage':
					require_once( $controllers_path . 'wcfmu-controller-wcbookings-resources-manage.php' );
					new WCFMu_WCBookings_Resources_Manage_Controller();
				break;
				
				case 'wcfm-bookings-settings':
					require_once( $controllers_path . 'wcfmu-controller-wcbookings-settings.php' );
  ...
  /**
	...
	function wcb_product_manage_availability_fields( $availability_fields, $product_id ) {
		...
		$wcfmu_availability_fields = array (
																				...
																				"_wc_booking_first_block_time" => array('label' => __('First block starts at...', 'woocommerce-bookings') , 'type' => 'time', 'class' => 'wcfm-text wcfm_ele booking', 'label_class' => 'wcfm_title booking', 'placeholder' => "HH:MM", 'value' => $first_block_time ),
  ...
  function wcb_wcfmu_products_manage_form_load_views( ) {
	 ...
	 require_once( $WCFMu->library->views_path . 'wc_bookings/wcfmu-view-wcbookings-products-manage.php' );
	}
	...
	function wcfm_message_on_new_booking( $booking_id ) {
  	...
  	$author_is_admin = 1;
		...
		if ( $booking_id ) {
			...
			$WCFM->frontend->wcfm_send_direct_message( $author_id, $message_to, $author_is_admin, $author_is_vendor, $wcfm_messages, 'booking' );
