...
class WCFMu_Vendor_Badges {
	...
	public function __construct() {
		...
		// Badges Settings
		add_action( 'end_wcfm_settings', array( &$this, 'wcfmu_vendor_badges_settings' ), 14 );
		add_action( 'wcfm_settings_update', array( &$this, 'wcfmu_vendor_badges_settings_update' ), 14 );
		...
		add_filter( 'wcfm_dashboard_username', array( &$this, 'after_wcfm_dashboard_user' ), 11 );
	...
	function wcfmu_vendor_badges_settings( $wcfm_options ) {
		...
		<div class="page_collapsible" id="wcfm_settings_form_vendor_badges_head">
			...
			<?php _e('Vendor Badges', 'wc-frontend-manager-ultimate'); ?><span></span>
		</div>
			...
			foreach( $this->wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option ) {
				if( isset( $wcfm_vendor_badges_option['is_active'] ) && !empty( $wcfm_vendor_badges_option['badge_name'] ) ) {
					$WCFM->wcfm_fields->wcfm_generate_form_field( array(  
																													"wcfm_membership_badges_".$badge_key => array( 'label' => '<img src="'.$wcfm_vendor_badges_option['badge_icon'].'" style="width: 32px; margin-right: 5px; display: inline-block;">'.$wcfm_vendor_badges_option['badge_name'], 'name' => 'wcfm_membership_badges['.$badge_key.']', 'type' => 'checkbox', 'class' => 'wcfm-checkbox wcfm_ele', 'label_class' => 'wcfm_title checkbox_title', 'value' => 'yes', 'dfvalue' => ( isset( $wcfm_membership_badges[$badge_key] ) ? 'yes' : 'no' ) )
																													) );
				}
			}
		}
	...
	function wcfmu_vendor_badges_manage( $vendor_id ) {
		...
		?>
		<!-- collapsible -->
		<div class="page_collapsible vendor_manage_badges" id="wcfm_vendor_manage_form_badges_head"><label class="fa fa-certificate"></label><?php _e( 'Badges', 'wc-frontend-manager-ultimate' ); ?><span></span></div>
		<div class="wcfm-container">
				...
				<div class="wcfm_vendor_badges_manage">
				  <form id="wcfm_vendor_manage_badges_form" class="wcfm">
						...
						foreach( $this->wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option ) {
							if( isset( $wcfm_vendor_badges_option['is_active'] ) && !empty( $wcfm_vendor_badges_option['badge_name'] ) ) {
								$WCFM->wcfm_fields->wcfm_generate_form_field( array(  
																																"wcfm_vendor_badges_".$badge_key => array( 'label' => '<img src="'.$wcfm_vendor_badges_option['badge_icon'].'" style="width: 32px; margin-right: 5px; display: inline-block;">'.$wcfm_vendor_badges_option['badge_name'], 'name' => 'wcfm_vendor_badges['.$badge_key.']', 'type' => 'checkbox', 'class' => 'wcfm-checkbox wcfm_ele', 'label_class' => 'wcfm_title checkbox_title', 'value' => 'yes', 'dfvalue' => ( isset( $wcfm_vendor_badges[$badge_key] ) ? 'yes' : 'no' ) )
																																) );
							}
						}
	 ...
	 */
	function show_wcfm_vendor_badges( $vendor_id = 0, $is_large = false ) {
		global $WCFM, $WCFMu;
			...
			if( !empty( $wcfm_vendor_badges ) ) {
				foreach( $this->wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option ) {
					if( isset( $wcfm_vendor_badges_option['is_active'] ) && !empty( $wcfm_vendor_badges_option['badge_name'] ) && isset( $wcfm_vendor_badges[$badge_key] ) ) {
						echo '<div class="'.$badge_classses.' text_tip"  data-tip="' . $wcfm_vendor_badges_option['badge_name'] . '"><img src="' . $wcfm_vendor_badges_option['badge_icon'] . '" /></div>';
					}
				}
			}
				...
				if( !empty( $wcfm_membership_badges ) ) {
					...
					foreach( $wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option ) {
						if( isset( $wcfm_vendor_badges_option['is_active'] ) && !empty( $wcfm_vendor_badges_option['badge_name'] ) && isset( $wcfm_membership_badges[$badge_key] ) ) {
							echo '<div class="wcfm_vendor_badge wcfm_vendor_badge_large text_tip"  data-tip="' . $wcfm_vendor_badges_option['badge_name'] . '"><img src="' . $wcfm_vendor_badges_option['badge_icon'] . '" /></div>';
						}
					}
					echo '</div>';
				}
	...
	function after_wcfm_dashboard_user( $username ) {
		global $WCFM, $WCFMu;
		if( empty( $this->wcfm_vendor_badges_options ) ) return $username;
		...
		if( !empty( $wcfm_vendor_badges ) ) {
			foreach( $this->wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option ) {
				if( isset( $wcfm_vendor_badges_option['is_active'] ) && !empty( $wcfm_vendor_badges_option['badge_name'] ) && isset( $wcfm_vendor_badges[$badge_key] ) ) {
					$username .= '<div class="wcfm_vendor_badge text_tip"  data-tip="' . $wcfm_vendor_badges_option['badge_name'] . '"><img src="' . $wcfm_vendor_badges_option['badge_icon'] . '" /></div>';
				}
			}
		}
		return $username;
	...
	function wcfm_dokan_product_seller_tab( $val ) {
		...
		if( $vendor_id ) {
			...
			if( !empty( $wcfm_vendor_badges ) ) {
				foreach( $this->wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option ) {
					if( isset( $wcfm_vendor_badges_option['is_active'] ) && !empty( $wcfm_vendor_badges_option['badge_name'] ) && isset( $wcfm_vendor_badges[$badge_key] ) ) {
						$author->display_name .= '<div class="wcfm_vendor_badge wcfm_vendor_badge_large text_tip"  data-tip="' . $wcfm_vendor_badges_option['badge_name'] . '"><img src="' . $wcfm_vendor_badges_option['badge_icon'] . '" /></div>';
					}
				}
			}
		...
		if( $vendor_id ) {
			...
			if( $vendor_admin_id ) {
				...
				if( !empty( $wcfm_vendor_badges ) ) {
					foreach( $this->wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option ) {
						if( isset( $wcfm_vendor_badges_option['is_active'] ) && !empty( $wcfm_vendor_badges_option['badge_name'] ) && isset( $wcfm_vendor_badges[$badge_key] ) ) {
							$name .= '<div class="wcfm_vendor_badge text_tip"  data-tip="' . $wcfm_vendor_badges_option['badge_name'] . '"><img src="' . $wcfm_vendor_badges_option['badge_icon'] . '" /></div>';
						}
					}
				}
				$name .= '</div>';
