...
class WCFMu_Reports_Low_In_Stock_Controller {
	...
	public function processing() {
		...
		$wcfm_low_in_stock_json = '{
															...
															"recordsFiltered": ' . $filtered_low_in_stock_count . ',
															"data": ';
		if(!empty($items)) {
			$index = 0;
			...
			foreach($items as $wcfm_product_single) {
				...
				if( $the_product->post->post_status == 'publish' ) {
					//$actions = ( current_user_can( 'edit_published_products' ) ) ? '<a class="wcfmu_product_quick_edit" href="#" data-product="'. $the_product->get_ID() . '"><span class="wcicon-link text_tip" data-tip="' . esc_attr__( 'Quick Edit', 'wc-frontend-manager-ultimate' ) . '"></span></a>' : '';
					...
					$actions .= ( current_user_can( 'edit_published_products' ) && apply_filters( 'wcfm_is_allow_edit_products', true ) ) ? '<a class="wcfm-action-icon" href="' . get_wcfm_edit_product_url($wcfm_product_single->id, $the_product) . '"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Edit', 'wc-frontend-manager-ultimate' ) . '"></span></a>' : '';
					$actions .= ( current_user_can( 'delete_published_products' ) && apply_filters( 'wcfm_is_allow_edit_products', true ) ) ? '<a class="wcfm_product_delete wcfm-action-icon" href="#" data-proid="' . $wcfm_product_single->ID . '"><span class="fa fa-trash-o text_tip" data-tip="' . esc_attr__( 'Delete', 'wc-frontend-manager-ultimate' ) . '"></span></a>' : '';
				} else {
					//$actions = ( current_user_can( 'edit_products' ) ) ? '<a class="wcfmu_product_quick_edit" href="#" data-product="'. $the_product->get_ID() . '"><span class="wcicon-link text_tip" data-tip="' . esc_attr__( 'Quick Edit', 'wc-frontend-manager-ultimate' ) . '"></span></a>' : '';
					$actions .= '<a class="wcfm-action-icon" target="_blank" href="' . get_permalink( $wcfm_product_single->id ) . '"><span class="fa fa-eye text_tip" data-tip="' . esc_attr__( 'View', 'wc-frontend-manager-ultimate' ) . '"></span></a>';
					$actions .= ( current_user_can( 'edit_products' ) && apply_filters( 'wcfm_is_allow_manage_products', true ) ) ? '<a class="wcfm-action-icon" href="' . get_wcfm_edit_product_url($wcfm_product_single->id, $the_product) . '"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Edit', 'wc-frontend-manager-ultimate' ) . '"></span></a>' : '';
					$actions .= ( current_user_can( 'delete_products' ) && apply_filters( 'wcfm_is_allow_manage_products', true ) ) ? '<a class="wcfm_product_delete wcfm-action-icon" href="#" data-proid="' . $wcfm_product_single->ID . '"><span class="fa fa-trash-o text_tip" data-tip="' . esc_attr__( 'Delete', 'wc-frontend-manager-ultimate' ) . '"></span></a>' : '';
				}
