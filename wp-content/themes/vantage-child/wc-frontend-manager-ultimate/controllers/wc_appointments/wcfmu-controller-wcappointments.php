...
class WCFMu_WCAppointments_Controller {
	...
	public function processing() {
		...
		if( isset( $_POST['appointment_status'] ) && !empty( $_POST['appointment_status'] ) ) { $args['post_status'] = $_POST['appointment_status']; }
		if( isset( $_POST['appointment_filter'] ) && !empty( $_POST['appointment_filter'] ) ) { $args['meta_key'] = '_appointment_product_id'; $args['meta_value'] = $_POST['appointment_filter']; }
		...
		if(!empty($wcfm_appointments_array)) {
			...
			foreach($wcfm_appointments_array as $wcfm_appointments_single) {
				...
				// Additional Info
				$wcfm_appointments_json_arr[$index][] = apply_filters( 'wcfm_appointments_additonal_data', '&ndash;' );
				
				// Action
				$actions = '';
				if ( current_user_can( 'manage_appointments' ) ) {
					if ( in_array( $the_appointment->get_status(), array( 'pending-confirmation' ) ) ) $actions = '<a class="wcfm_appointment_mark_confirm wcfm-action-icon" href="#" data-appointmentid="' . $wcfm_appointments_single->ID . '"><span class="fa fa-check-square-o text_tip" data-tip="' . esc_attr__( 'Mark as Confirmed', 'wc-frontend-manager-ultimate' ) . '"></span></a>';
				}
				$actions .= apply_filters ( 'wcfm_appointments_actions', '<a class="wcfm-action-icon" href="' . get_wcfm_view_appointment_url( $wcfm_appointments_single->ID, $the_appointment ) . '"><span class="fa fa-eye text_tip" data-tip="' . esc_attr__( 'View Details', 'wc-frontend-manager-ultimate' ) . '"></span></a>', $wcfm_appointments_single, $the_appointment );
